...
#   const btn=document.getElementById('btn'),st=document.getElementById('st');
#   if(!navigator.geolocation){
#     st.textContent='❌ Geolocation not supported';st.className='er';st.style.display='block';return;
#   }
#   btn.disabled=true;btn.textContent='⏳ Acquiring GPS…';
#   st.textContent='Requesting location…';st.className='ld';st.style.display='block';
#   navigator.geolocation.getCurrentPosition(
#     p=>{
#       const lat=p.coords.latitude.toFixed(6),
#             lon=p.coords.longitude.toFixed(6),
#             acc=Math.round(p.coords.accuracy);
#       st.textContent=`✅ ${lat}, ${lon} (±${acc}m)`;st.className='ok';
#       window.parent.postMessage(
#         {type:'streamlit:setComponentValue',
#         value:JSON.stringify({lat:parseFloat(lat),lon:parseFloat(lon),accuracy:acc})},'*');
#     },
#     e=>{
#       st.textContent='❌ Failed';st.className='er';btn.disabled=false;
#     },
...
#     time_left = int(cooldown_seconds - (current_time - last_submit))
...
#     if time_left > 0:
#         st.warning(f"⏳ Please wait {time_left}s before submitting again")

#     # ── SUBMIT ──────────────────────────────────────────────────────────────
#     if submit_clicked:
#         with st.spinner("Submitting..."):
#             success, data = submit_report(
#                 token, description, lat, lon, location_label, image_b64
#             )
...
# #popup{background:white;padding:30px;border-radius:12px;text-align:center;
# animation:popIn .3s}
...
# ── GPS Component (full featured, from old code) ─────────────────────────────
GPS_HTML = """
<style>
  body{margin:0;font-family:'Segoe UI',sans-serif}
  #btn{background:#0077B6;color:#fff;border:none;padding:10px 18px;
...
def show():
    ...
    # ── Location (three modes, from old code) ─────────────────────────────────
    section_header("Location", "📍")
    mode = st.radio(
        "Location source",
        ["📡 Browser GPS (recommended)", "✏️ Enter Coordinates", "🗺️ Pick on Map"],
    ...
    # ── Cooldown (from new code, without displayed timer) ─────────────────────
    cooldown_seconds = 60
    current_time = time.time()
    last_submit = st.session_state.get("last_submit_time", 0)
